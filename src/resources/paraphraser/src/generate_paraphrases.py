...
def main(args):
        ...
        for sample in t:
            ...
            for i, sample_id in enumerate(sample['id'].tolist()):
                ...
                for j, hypo in enumerate(hypos[i][:args.nbest]):
                    ...
                    prism_paraphrases[sample_id]=hypo_str
    ...
    prism_paraphrases = collections.OrderedDict(sorted(prism_paraphrases.items()))
    prism_paraphrases= list(prism_paraphrases.values())
    ...
    for key in data.keys():
        candidates={}
        candidates["context"] = data[key]["context"]
        candidates["question"] = data[key]["question"]
        candidates["question_para"] = prism_paraphrases[i]
        i=i+1
        candidates["question_variant"]=data[key]["question_variant"]
        candidates["question_variant_para"] = prism_paraphrases[i]
        i=i+1
        candidates["score"] = data[key]["score"]
        new_data[key]=candidates

    dict_ner_candidates = json.dumps(new_data, indent=4, sort_keys=True)
    with open('generated_questions_paraphrased.json', 'w') as outfile:
        outfile.write(dict_ner_candidates)
    '''
